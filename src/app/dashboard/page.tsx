      ...
      <main className="pb-20 pt-10">
        <Container className="max-w-4xl">
          <div className="rounded-[2rem] border bg-white/85 p-8 text-center shadow-[0_24px_70px_rgba(55,34,23,0.08)]">
            <p className="text-sm uppercase tracking-[0.3em] text-muted">Customer dashboard</p>
            ...
            {snapshot.orders.length ? (
              snapshot.orders.map((order) => (
                <Link
                  ...
                  className="block rounded-[1.6rem] border bg-[#fffaf6] p-5 hover:bg-white"
                >
          ...
          <div className="space-y-4 rounded-[2rem] border bg-white/80 p-6 shadow-[0_18px_45px_rgba(58,35,25,0.06)]">
            ...
            {snapshot.consultations.length ? (
              snapshot.consultations.map((consultation) => (
                <div key={consultation.id} className="rounded-[1.6rem] border bg-[#fffaf6] p-5">
                  <p className="font-semibold text-foreground">
                    {consultation.service?.name ?? "General design consultation"}
                  </p>
                  <p className="mt-1 text-sm text-muted">
                    {formatStatus(consultation.status)} Â· {consultation.channel.replaceAll("_", " ")}
                  </p>
                  <p className="mt-3 text-sm text-muted">
                    Requested for {formatDate(consultation.requestedDate)} Â· {consultation.preferredTimeSlot}
                  </p>
                </div>
              ))
            ) : (
            ...
            <div className="rounded-[1.6rem] border bg-[#2a211d] p-5 text-white">
